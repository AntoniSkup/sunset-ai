...
export default function DashboardLayout({
  ...
  // Sign-out triggers a full app navigation back to "/" — we use the bare
  // next/navigation router rather than the locale-aware one because we
  // explicitly want the post-logout landing to drop any locale subpath.
  ...
  const navItems = [
    { href: "/dashboard", icon: UsersIcon, label: tNav("team") },
    {
      href: "/dashboard/payments",
      icon: CreditCardIcon,
      label: tNav("payments"),
    },
    { href: "/dashboard/general", icon: Cog6ToothIcon, label: tNav("general") },
    {
  ...
  return (
    <div className="relative flex h-full min-h-0 w-full flex-col overflow-hidden bg-white">
      ...
      <div className="relative z-10 flex min-h-0 flex-1 overflow-hidden">
        ...
        <main className="w-full flex-1 overflow-y-auto p-0 lg:p-4">
          {children}
        </main>
      </div>
    </div>
