...
type TimeTranslator = ReturnType<typeof useTranslations<"builder.time">>;

function getRelativeTime(dateStr: string, t: TimeTranslator, locale: string) {
  ...
  if (diffInSeconds < 86400) {
    ...
    return hours === 1 ? t("anHourAgo") : t("hoursAgo", { count: hours });
  }
  if (diffInSeconds < 604800) {
    const days = Math.floor(diffInSeconds / 86400);
    return days === 1 ? t("aDayAgo") : t("daysAgo", { count: days });
  }
...
export default function StartPage() {
  ...
  const tTime = useTranslations("builder.time");
  const tSuggestions = useTranslations("builder.suggestions");
  const tUpload = useTranslations("builder.uploadToast");
  ...
  const typingTexts = t.raw("placeholderTyping") as string[];
  const dateLocale =
    ...
    try {
      ...
      if (startedUploads || uploadToast?.status === "uploading") {
        setUploadToast({
          ...
          message:
            error instanceof Error ? error.message : tUpload("uploadFailed"),
        });
