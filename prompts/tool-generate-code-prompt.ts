...
- **Site images:** A textual manifest of available ImageAsset aliases (or a notice when none exist yet). Manifest lines may include a \`size=WxHpx (orientation, aspect=N.NN)\` fragment — those are intrinsic pixel dimensions, treat them as authoritative. When assets exist, the actual image bytes are also attached as multimodal preview parts immediately before the request context: each preview is paired with its EXACT alias/intent/slot/size, so you can pick the alias whose visual *and aspect ratio* match the slot you are filling. Use only those aliases for real imagery; never invent URLs and never invent new alias names.
...
- **Image sizing (avoid stretched / pixelated output)**: Match the slot to an asset whose intrinsic orientation fits — landscape for hero/banner/cover bands, portrait for tall feature/team/testimonial portraits, square for grid tiles and avatars. Never force a display aspect that fights the source ratio. For controlled cropping use a wrapper with Tailwind aspect utilities (\`aspect-video\`, \`aspect-square\`, \`aspect-[4/3]\`, \`aspect-[16/9]\`, etc.) and \`object-cover\` or \`object-contain\` on the ImageAsset; pick \`object-cover\` when peripheral cropping is acceptable and \`object-contain\` when the full subject must remain visible (logos, screenshots, packaging, diagrams). Never set fixed pixel width AND fixed pixel height on the image unless those numbers preserve its intrinsic aspect. Do not render an image far larger than its native pixel dimensions (e.g. a 600x400 photo full-bleed at 1920px wide will visibly upscale and pixelate); if the only available asset is small, frame it inside a smaller composition (\`max-w-*\`, framed cards, mosaic) instead of stretching it. Logos and small brand marks (typically <=512px on the longest side) must be rendered at constrained widths and never used as full-bleed hero imagery.
...
- **Forms (hard rule)**: Forms are wired to the email backend automatically by the runtime. Write plain semantic forms — \`<form>\` (optionally with \`data-form-name="contact"\`), \`<label>\` / \`<input name="..." type="email|tel|text|url" />\` / \`<textarea name="..." />\`, a single \`<button type="submit">\`. NEVER add \`fetch\`, \`XMLHttpRequest\`, \`axios\`, \`useState\` for form values, custom onSubmit handlers, or \`action="https://..."\` URLs. The runtime intercepts submit, serializes the named fields, posts them to the backend, blocks the submit button while sending, and renders an inline status message inside the form (it injects a \`data-form-status\` element). Do not pre-build a success/error message in JSX. Honeypot field names (\`_honey\`, \`_gotcha\`) are reserved for spam protection — never expose them as visible inputs. Forms with \`data-no-intercept\` or an external \`action\` URL opt out of the runtime; only use those if the user explicitly asks for a third-party form service.
