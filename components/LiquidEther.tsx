...
// Sunset gradient colors for light theme
const sunsetColorsLight = ['#FF6B35', '#FF8C94', '#FFD23F', '#FFA07A', '#FF8C69'];

// Sunset gradient colors for dark theme
const sunsetColorsDark = ['#FF4500', '#FF1493', '#FF8C00', '#FF6347', '#FF69B4'];

function useTheme(theme: 'light' | 'dark' | 'auto' = 'auto'): 'light' | 'dark' {
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // Determine colors to use based on theme and props
    const colorsToUse = colors || (useSunsetColors
      ? (currentTheme === 'dark' ? sunsetColorsDark : sunsetColorsLight)
      : defaultColors);
    const paletteTex = makePaletteTexture(colorsToUse);
...
`;
    const color_frag = `
		...
		// Use a power curve to preserve color saturation at low velocities
		// Lower exponent (0.5-0.6) keeps colors vibrant even at edges
		float intensity = pow(lenv, 0.55);
		// Add a minimum brightness floor to prevent grayish edges
		// This ensures colors remain visible and saturated even at very low velocities
		intensity = max(intensity, lenv * 0.5);
		vec3 outRGB = c * intensity;
		float outA = lenv;
